...
export default function AdminArticles() {
  ...
  const [formData, setFormData] = useState({ title: '', description: '', content: '', category: '', image: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (currentId) {
        await updateDoc(doc(db, 'articles', currentId), formData);
      } else {
  ...
  const handleEdit = (article: Article) => {
    setFormData({ title: article.title, description: article.description, content: article.content || '', category: article.category, image: article.image || '' });
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => { setIsEditing(true); setCurrentId(null); setFormData({ title: '', description: '', content: '', category: '', image: '' }) }} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded flex items-center gap-2">
      ...
      {isEditing ? (
        <div className="bg-white/5 p-6 rounded-2xl border border-white/10 mb-8">
          <form onSubmit={handleSubmit} className="space-y-4 text-black">
            ...
            <input required placeholder="Category" value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})} className="w-full p-2 border rounded text-slate-800" />
