...
import { Search, Mail, CheckCircle, Clock, Trash2, RefreshCw } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white/5 backdrop-blur-md rounded-2xl p-4 border border-white/10 text-center">
          ...
          <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Unread</div>
        </div>
        <div className="bg-white/5 backdrop-blur-md rounded-2xl p-4 border border-white/10 text-center">
          <div className="text-2xl font-bold text-[#2563EB]">{messages.filter(m => m.status === 'read').length}</div>
          <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Read</div>
        </div>
        <div className="bg-white/5 backdrop-blur-md rounded-2xl p-4 border border-white/10 text-center">
          <div className="text-2xl font-bold text-[#10B981]">{messages.filter(m => m.status === 'replied').length}</div>
          <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Replied</div>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <button onClick={fetchMessages} className="p-2 rounded-lg bg-white/5 border border-white/10 hover:bg-white/10 transition-all text-slate-400 hover:text-white" title="Refresh">
      ...
      {loading ? (
        ...
        <div className="bg-white/5 backdrop-blur-xl rounded-3xl p-12 border border-white/10 text-center">
          ...
          <p className="text-slate-400 text-sm">{messages.length === 0 ? 'No contact messages received yet.' : 'No messages match your filters.'}</p>
        </div>
      ) : (
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[10px] text-slate-500 mt-0.5">{new Date(msg.createdAt).toLocaleString('en-BD')}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  {msg.status !== 'replied' && (
                    <button onClick={(e) => { e.stopPropagation(); markAsReplied(msg.id); }}
                      className="p-1.5 rounded-lg bg-[#10B981]/10 border border-[#10B981]/30 text-[#10B981] hover:bg-[#10B981]/20 transition-all" title="Mark Replied">
